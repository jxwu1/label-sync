...
Save this to `scripts/screenshot.py` in the project root.
...
Examples:
  python scripts/screenshot.py docs/design-demos/01-dashboard.html screenshots/ref.png --theme=dark
  python scripts/screenshot.py http://localhost:5000/ screenshots/live.png --theme=dark
  python scripts/screenshot.py --diff screenshots/ref.png screenshots/live.png screenshots/diff.png
  python scripts/screenshot.py --audit http://localhost:5000/ --selectors ".sidebar,.header,.pnl,.btn,.tbl th"
"""
...
def diff_images(ref_path, live_path, diff_path):
    ...
    # Red overlay on mismatched pixels
...
```bash
pip install playwright Pillow numpy --break-system-packages
playwright install chromium
```

Playwright should already be installed in the project. PIL and numpy are needed only for the diff mode.
