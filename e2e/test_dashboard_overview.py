...
（label + value，x-init fetch 填充）。这里测当前契约两态：未初始化(—) / 已导入(数字)。
"""
...
def test_dashboard_uninitialized_shows_dash(live_server, page_with_console) -> None:
    """空沙箱 → /api/dashboard 返回 '—'，Dashboard stockpile 指标显示 '—'。"""
...
def test_dashboard_initialized_shows_counts(live_server, page_with_console) -> None:
    """import 数据 → Dashboard 库存 SKU=3 / 停用=0 / 上次导入为日期。"""
    page = page_with_console
