...
- **2026-05-09 范围收紧**：原描述与 `e2e/README.md`「不要把 e2e 当成 Playwright 单元测试库使」立场冲突。改为只补**只有真浏览器才能验**的回归（CSS specificity / Alpine 时序 / popover bbox 越界 / 浮层真 visibility），不补"store 状态可验"那部分（fill 时段后 save 这条留 Vitest）。
- **进度**：
  - [x] popover open + 真 visibility + bbox 在 viewport 内 (`e2e/test_attendance_popover.py`)
  - [x] holiday 导入后 cell 真渲染（class + computed style dot opacity + cursor + 同月非假日对照）(`e2e/test_attendance_holiday.py`)
  - [~] batch 选择 / fill-rates 渲染 —— 移出 e2e，留给将来 Vitest
- 估时：原 1d → 实际 ~1.5h（范围砍半，两块各 30min）
- **R1 完结**
...
### 2026-05-09 重定方案：lazy load on first nav switch（不是 idle preload）

聊下来用户真实痛点是"切到 sa 必须点刷新才出下半页"，不是"站启动就要把所有数据预热"。改方案：

- **架构**：nav store 加 `onFirstActivate(pageId, cb)` 钩子（roadmap §阶段 2 备忘"scan-history 进入触发"早就标过该加）。`switch(id)` 内部首次切到该 page 时触发回调一次，后续切走再回不重复
- **范围**：sa（saRefresh）+ dq（dqRefresh）。fc 实测已经在 IIFE 末尾 `await loadMonth()`、本来就 auto-load，**不动**
- **缓存语义**：首次 load 后不再自动重 load。用户做了改动按刷新走"强制重拉"路径
- **inv ×2 / dd 不加**：用户明确不需要预加载

**进度**：
- [x] PR1 Stream B 完结（store.js + sa.js + dq.js + 3 e2e case）
- [x] PR2 Stream A 完结：数据仓库 stockpile panel 状态 tab 2×2 mini stat
  - 后端扩 `/stockpile/status` 加 `active_count` / `inactive_count` / `last_import_at`（向后兼容保留 `count`=active）
  - 前端 `#spStats` + 4 数字（总条数 = active+inactive 前端聚合 / 上次 import "YYYY-MM-DD HH:MM" / 活跃 / 失效）
  - 既有 `initStockpile()` 末尾本来就调 `refreshSpStatus()`，进首页就显示 — 不需要新加触发
  - CSS `.sp-stats` 2×2 grid + `.sp-stat-num` mono + accent 色
  - 后端 +1 case (test_status_uninitialized 字段更新 + test_init_imports_csv_and_status_reflects 加 active/inactive/last_import 断言) + 2 e2e case (uninitialized hint / initialized 4 stats + CSS 真生效)

**估时**：原 0.5-1d → 实际 Stream B ~1h、Stream A ~1.5h，**全完结**
