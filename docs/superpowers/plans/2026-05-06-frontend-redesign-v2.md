...
## 待办：PR-FE-4b — 历史时间线合并 inventory_events（用户验证发现，2026-05-07）

**问题**：4a 落地后 timeline 经常空白。原因：后端 `aggregate_events` 只读 `stockpile_changes` 表，多数条码自创建后无修改 → 0 条事件。设计稿期望的 `sale / purchase / audit` 类型事件**当前后端不支持**。

**确认**：后端架构上 sale/purchase 在 `inventory_events` 表，与 `stockpile_changes` 是两套数据流，从未合并暴露过。

**实施方案**：
1. 后端：新建 `history_service.aggregate_full_timeline(barcode)` 合并 stockpile_changes + inventory_events 两源，按时间倒序，event_type 填 `sale / purchase` 区分
2. 前端：history.js timeline render 加 `sale / purchase` 两类 dot 颜色（已预留 data-type 机制，只需加 CSS rule）
3. CHANGE_TYPE_CN 表加 sale/purchase 中文映射

**估时**：0.5 天（后端 200-300 行 + 前端 ~30 行 CSS/JS）

**触发时机**：PR-FE-4c（5b/5c 视觉换皮）之后再做，或并入 PR-FE-9 收尾批次。
