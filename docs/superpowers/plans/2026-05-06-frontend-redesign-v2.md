...
**R2：`/attendance/fill-rates/<month>` N+1 query**
- 问题：循环员工每人调一次 `compute_summary`（routes_attendance.py:127）。20 员工 = 20 独立 session。
- 当前规模可接受，但员工数 > 50 性能差，且每次切月都触发。
- 改进：单次 SQL 聚合（按 employee_id 分组统计 absent/normal/leave/sunday days），或加 LRU 缓存（key = employee + month）。
- 估时：0.5d

### 中优先 — 边界 / 维护

**R3：`_GR_HOLIDAYS_BY_YEAR` 跨年维护**
- 数据写死 2025/2026 两年。2027 年初没人手动加 → 节假日导入失效。
- 改进选项：
  - a. 改 JSON 配置文件 `config/gr-holidays.json`，更易批量编辑/PR review
  - b. CI lint：`year + 1 not in dict and now() > year-11-01` 报警
  - c. 文档明确「年初维护事项」清单
- 估时：0.5d（任选）

**R4：alembic env 用相对路径 sqlite:///stockpile.db**
- 测试 / 调试期间多次意外触发 prod DB 写入（如 inventory_imports 表创建那次）。
- 改进：alembic env.py 读 `os.environ.get('DB_PATH', 'stockpile.db')`，pytest conftest 设 env var 隔离。
- 估时：30 min

**R5：PR-FE-7c batch operations 串行**
- 选 30 天 + 网络慢 → 3-5s 等待。
- 改进：`Promise.allSettled` 并行（backend 写入幂等）。
- 估时：30 min
