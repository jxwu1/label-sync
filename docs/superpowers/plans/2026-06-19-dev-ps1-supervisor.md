...
$apiProc = Start-Supervised 'python' @('-u', 'server.py') $root @{ PYTHONUNBUFFERED = '1' }
...
try {
    while ($true) {
        ...
        if ($exited) {
            $exitCode = $exited.proc.ExitCode   # 先存，避免被 taskkill 的 $LASTEXITCODE 覆盖
            Write-Host "[$($exited.name)] exited (code $exitCode)" -ForegroundColor Yellow
            break
        }
...
# Ctrl+C 走到这里：PowerShell finally 已执行清理；显式退 130；正常退出走 $exitCode
if ($exitCode -eq 0) { exit 0 } else { exit $exitCode }
```

> **Ctrl+C 130**：PowerShell 在 Ctrl+C 时会执行 `finally`（清理跑到）。脚本被 Ctrl+C 中断时进程退出码由运行时决定；为显式保证 130，实现时在脚本顶部加 `Console.CancelKeyPress` 处理或在被中断路径 `exit 130`。**实现要点（实施者对照真实 pwsh 验证）**：若 try 循环被 Ctrl+C 打断后控制权不回到末尾 `exit`，需在循环中检测取消并 `exit 130`，或注册 `[Console]::CancelKeyPress` 事件设标志后 break + `exit 130`。以验收项②（Ctrl+C 后退出码=130 且无残留）为准迭代。
