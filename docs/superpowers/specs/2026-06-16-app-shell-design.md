...
- 简报并入壳：前端 `/ui/briefing` 从裸路由变成壳的子路由（与 Flask 旧 `/briefing` 302 路由无关）。

**Out（明确不做）**
- 货号历史及其它页迁移（各自后续 spec）。
- substrip（SESSION/OPERATOR/WAREHOUSE/STOCKPILE 那条扫描工作流横条）—— **砍掉，不复刻**。
- 主题持久化到服务端（见 §8.3，改为浏览器本地）。
- **键盘快捷键**（旧栈 `Ctrl/Cmd + 0-9` 切 nav）+ 侧栏 nav-kbd 角标 —— **有意删减**，不复刻（决策 B）。NavItem 不含 `shortcut` 字段，壳不注册全局 keydown。
...
- **`SidebarNav.vue`**：消费 `nav-items.ts` + 当前路由 + `useCurrentUser`。渲染 "MODULES · N" + 列表。每项：
  ...
  - active：有 `routeName` 的项按当前 route 高亮；未迁项不高亮（用户在旧栈时本组件不渲染）。
...
**前端 unit（vitest）**
- `SidebarNav`：渲染 N 项 + "MODULES · N"；active 高亮当前 route；`!isAdmin` 隐藏 requiresAdmin 项，`isAdmin` 显示；有 `routeName` 的项渲染 `RouterLink`、未迁项(`legacyPageId`)渲染 `<a href="/?page=id">`（href 无空格）。
...
4. `git diff --stat main...HEAD -- app/` 仅 `auth.py`（加 scanner 例外）+ `schemas_api.py`（加 MeData）+ 新 `app/routes/me.py` + `app/routes/__init__.py`（import + register blueprint）；无其它后端改动。
