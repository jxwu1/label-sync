...
**状态：** 设计待批（2026-06-18）。两轮审查 REQUEST_CHANGES 已处置（第一轮 #1-8：守卫假保护/hasData/负净高度/step 折线/同价/tooltip；第二轮 #9-12：负净退货可命中标记 + X 共享日期域 + 措辞 + 窄容器）。待用户审阅后落 plan。
...
- **负净月（净 < 0）**：柱高 0 → 没有可悬停区域，故在 baseline 处该月 X 位置画一个**可命中的 warn 色标记**（短横线或小三角，有实际宽高），`<title>` =「{month} 月：净退货 {|净|} 件」挂在该标记上。解决红队场景（单负净月无采购 → hasData=true 但零高柱无 tooltip）。
...
- 进价折线/dot：点 X = `x(weekStart)`（或周中点，二选一写死，全程一致）。
- X 轴月份标签：取 ~7 个 monthStart，标签 X = `x(monthStart)`；**首尾标签须落在 viewBox 内**（padL/padR 留边，标签 text-anchor 端点处收边）。
...
- 折线：**真 step path**（HC-P3-8），点 X = `x(weekStart)`；`sameValue` → 中段水平线。
...
- `TimelineChart.test.ts`（组件，重点）：
  ...
  - **只有采购无销售** → 仍画折线（无柱）。
  - **只有销售无采购** → 画柱，无折线/dot。
  - **负净销量月不产生负 height + 退货标记可命中**（断言无 rect height < 0；该月有一个带 `<title>` 含「净退货」的可命中 warn 标记元素，而非仅零高柱）。
  - **红队场景**：单负净月、无采购 → hasData=true（不显「无数据」）且存在可命中退货标记。
  - **两次不同进价 → path 含阶梯跳变**（断言 path 有垂直段 / 直角，非单纯斜线）。
  - **同价分支** → 折线中段 + 右轴单 tick。
  - **X 共享日期域对齐**（HC-P3-9）：给定已知日期的月柱 + 落在该月日历范围内的采购周，断言该进价点 X 落在该月柱的 X 区间内（证明同一时间域，无索引漂移）。
  - **窄容器**：小 viewBox/宽度下，X 轴首、尾标签的 x 仍在 `[0, W]` 内（不溢出）。
