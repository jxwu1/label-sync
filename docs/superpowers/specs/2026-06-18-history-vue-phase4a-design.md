...
**状态：** 设计待批（2026-06-18）。第一轮审查 REQUEST_CHANGES 已处置（B1 单事务 get_batch_detail / B2 首加载链 ensureLoaded / B3 batch_id 存在性 404 / mode 严格 filter 宽松 / onDrill 设输入 / 红队 MEDIUM 服务端 cap+total_count+性能验收）。
...
**HC-4A-7（下钻）：** 最近改动变更行点击 → `onDrill(barcode)`：①切 `activeTab='search'`；②**先把搜索输入 ref 设为 barcode**（`query.value = barcode`，coder 核实查询 tab 实际 ref 名）；③再调搜索触发（`runSearch(barcode)` / `doSearch`）。现有 `runSearch` **不会**自动更新输入框，必须显式赋值，否则用户看到结果但输入框空/旧值。
...
def get_batch_detail(batch_id, mode="collapsed", filter_field=None, filter_change_type=None):
    ...
    with stockpile_db._session() as session:
        ...
        if batch_id != _OPEN_BATCH_ID:
            ...
            if ok is None:
                return None
        start, end = _batch_window(session, batch_id)   # 同 session 复用
        # summary 全量（filter 无关）+ changes（filter + mode）同窗口同读
        summary = _summarize(_fetch_window_rows(session, start, end))
        raw = _fetch_window_rows(session, start, end, filter_field, filter_change_type)
        changes_full = _shape_changes(session, raw, mode)   # collapsed 折叠 / raw 原样 + model join
        total_count = len(changes_full)
        return {"summary": summary, "changes": changes_full[:_RC_MAX_ROWS], "total_count": total_count}
```
> coder：把现有 `get_batch_summary` / `get_batch_changes` 的「查窗口 rows / _summarize / collapsed 折叠 / model join」逻辑抽成同 session 内的 helper（`_fetch_window_rows` / `_shape_changes`），`get_batch_detail` 复用。**旧 `get_batch_summary` / `get_batch_changes` / 旧路由保持不变**（旧页用）。`_batch_window` 已接受 `_OPEN_BATCH_ID`。需 import `StockpileSnapshot` / `and_` / `select`（文件已有）。
...
独立 pinia store：
- state：`batches`、`selectedBatchId`、`summary`、`changes`、`totalCount`、`mode`('collapsed')、`filter`({field,changeType})、`loading`/`error`（batches 级）、`detailLoading`/`detailError`（per-batch 级）、`loaded`(bool, 首加载幂等闸)、闭包 `let seq=0`。
- **`ensureLoaded()`（单一入口，幂等）**：`if (loaded) return;` → `loaded=true` → `await loadBatches()`。**组件首次激活只调 `ensureLoaded()`**，回切 tab 不重复加载（`loaded` 已 true）。
- `loadBatches()`：拉 `/api/history/recent-changes/batches`，填 batches；**若非空，自动 `selectBatch(batches[0].batchId)`**（首加载链终点——loadBatches 负责选首项并触发首次 detail，责任唯一）；空则清 summary/changes。
- `loadDetail()`：`const my=++seq`；`detailLoading=true`；拉 `/api/history/recent-changes/${bid}/changes?mode&field&change_type`；await 后 `if(my!==seq)return`；写 summary/changes/totalCount；catch `if(my!==seq)return` + 401 吞 + 写 detailError；finally `if(my===seq)` 守 detailLoading。
- `selectBatch(id)`：set selectedBatchId + 清 filter → `loadDetail()`。`setMode(m)` / `setFilter(f)`：set 后 → `loadDetail()`。`reset()`：`seq++` + 清空全部 state（含 `loaded=false`）。
- **首加载不变量（测试锁）**：组件首次激活 → 恰好 1 次 `/batches` + 1 次 `/changes`；回切 tab → 0 次新请求。
...
- `onDrill(barcode)`（HC-4A-7）：`activeTab='search'`；**先 `query.value = barcode`**（核实查询 tab 实际输入 ref 名）；再 `runSearch(barcode)`。
...
- **单事务一致性（HC-4A-2）**：`/changes` 经 `get_batch_detail` 单 session；断言 summary 计数与 changes（全量 total_count）口径一致（同窗口）。
...
- `recentChanges.test.ts`（store）：loadBatches 填 + **自动选首项触发 loadDetail** / loadDetail 填 summary+changes+totalCount / 失败填 detailError / 401 吞 / setMode·setFilter 重拉 / **HC-B7：批次 A→B 快切 A 后到不覆盖 B** / reset 作废 pending + `loaded=false` / **`ensureLoaded` 幂等：首次调 → 恰好 1 次 batches + 1 次 changes；二次调 → 0 次新请求**。
