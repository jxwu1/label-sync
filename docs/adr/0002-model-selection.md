...
- **wholesale 收益被 dying 闸压低**（量化）：活跃 SKU 最后销售距今 13-26 周的
  ...
  **→ 已实施（2026-06-11 同日补丁）**：dying 类型感知 —— 深度死亡（≥26 周）不分型
  短路；边际带 [13,26) 内仅 wholesale_only 存活。边际带构成实测：
  retail 1,140（按设计仍判死）/ **wholesale 32（获救）** / mixed 27（仍判死）；
  其中 10 个过非零周闸 → wholesale 预测 39→49。**这是语义正确性修复而非
  覆盖率跃升** —— 边际带主体本就是该死的零售慢销，量化否决了"翻几倍"的直觉。
  已知边界：Croston 对长零尾不衰减（边际带 SKU 的周字段可能偏旧），补货量由
  bootstrap horizon 列兜底（零周稀释分位数）；若未来要精细处理，TSB 是升级路径（D4）。
