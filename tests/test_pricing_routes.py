...
class PricingRoutesTests(unittest.TestCase):
    ...
    def test_preview_ok(self):
        payload = {
            "rows": [{"barcode": "OLD1", "price": 1.05, "quantity": 50}],
            "new_entries": [],
            "supplier_id": "S1",
            "supplier_name": "X",
        }
        with mock.patch(
            "app.routes.purchase.pricing_sheet.preview_pricing",
            return_value={
    ...
    def test_export_returns_xlsx(self):
        ...
        wb.save(buf)
        with mock.patch(
            "app.routes.purchase.pricing_sheet.export_pricing_bytes", return_value=buf.getvalue()
        ):
            res = self.client.post(
                "/purchase/pricing/export",
                json={
                    "rows": [],
                    "new_entries": [],
                    "supplier_id": "S1",
