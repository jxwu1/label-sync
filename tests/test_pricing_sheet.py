...
class TestMedianAndTarget(unittest.TestCase):
    ...
    def test_target_margin_no_supplier_or_no_data(self):
        self.assertEqual(
            pricing_sheet.compute_target_margin_pct([], ""), {"median": None, "n_samples": 0}
        )
        self.assertEqual(
...
class TestBuildXlsx(unittest.TestCase):
    ...
    def test_layout_formulas_and_format(self):
        ...
        new_r = next(
            r for r in range(1, ws.max_row + 1) if ws.cell(row=r, column=2).value == "NEW1"
        )
        ...
        # ---- 调价老品行：真实老利润率 + 推荐新售价 ----
        old_r = next(
            r for r in range(1, ws.max_row + 1) if ws.cell(row=r, column=2).value == "OLD1"
        )
    ...
    def test_changed_without_saleprice_falls_back_to_mirror(self):
        ...
        wb = openpyxl.load_workbook(io.BytesIO(data))
        ws = wb.active
        r = next(rr for rr in range(1, ws.max_row + 1) if ws.cell(row=rr, column=2).value == "OLD9")
    ...
    def test_free_goods_zero_oldprice_falls_back_to_mirror(self):
        ...
        ws = wb.active
        r = next(
            rr for rr in range(1, ws.max_row + 1) if ws.cell(row=rr, column=2).value == "FREE1"
        )
    ...
    def test_name_formula_injection_neutralized(self):
        ...
        wb = openpyxl.load_workbook(io.BytesIO(data))
        ws = wb.active
        row = next(r for r in range(1, ws.max_row + 1) if ws.cell(row=r, column=2).value == "X1")
...
class TestOrchestration(unittest.TestCase):
    def _fixture(self):
        ...
        p1 = mock.patch.object(
            pricing_sheet.stockpile_db, "query_products_by_barcodes", return_value=products
        )
        p2 = mock.patch.object(
            pricing_sheet.analytics_service, "list_sku_summary", return_value=summary
        )
