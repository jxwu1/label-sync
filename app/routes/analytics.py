...
def sku_metrics(barcode: str):
    ...
    try:
        ...
        # 单页取数去重: 取一次全量事件行, 供 extras/holding/heatmap 复用 (原各取一次共 3 次)
        event_rows = analytics_service.fetch_event_rows(barcode)
        extras = analytics_service.compute_sku_extras(barcode, rows=event_rows)
        holding = analytics_service.compute_avg_holding_days(barcode, rows=event_rows)
        heatmap = analytics_service.compute_monthly_heatmap(barcode, rows=event_rows)
