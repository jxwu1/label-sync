...
def _build_employee_block(emp: dict, summary: dict) -> list:
    anchor_para = Paragraph(
        f'<a name="{_employee_anchor(emp["id"])}"/>', getSampleStyleSheet()["Normal"]
    )
...
def build_pdf(month: str) -> bytes:
    ...
    if not employees:
        ...
        summaries = {
            emp["id"]: attendance_service.compute_summary(emp["id"], month) for emp in employees
        }
