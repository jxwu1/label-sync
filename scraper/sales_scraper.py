"""
boson ERP 销售明细抓取 (按月切片 + 缓存 + parquet/xlsx 输出).

用法:
  python scraper/sales_scraper.py                  # 默认最近 1 年
  python scraper/sales_scraper.py --from 2023-05-12
  python scraper/sales_scraper.py --from 2023-05-12 --to 2026-05-12

输出: SCRAPE_OUTPUT_DIR/events_sale_<from>_<to>.parquet|xlsx
缓存: SCRAPE_OUTPUT_DIR/_cache/sale/<begin>_<end>.parquet

配置: 从 scraper/.env 读 (BOSON_BASE_URL / SCRAPE_COOKIE_FILE / SCRAPE_OUTPUT_DIR).
cookie 失效: 浏览器登录 boson, F12 复制 PHPSESSID 到 scraper/cookie.txt.
"""
import argparse
import os
import re
...
CACHE_DIR = OUTPUT_DIR / "_cache" / "sale"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

# argparse 在 __main__ 里会覆盖这俩
DATE_FROM: date = date.today() - relativedelta(years=1)
DATE_TO: date = date.today()
OUTPUT_PARQUET = OUTPUT_DIR / f"events_sale_{DATE_FROM}_{DATE_TO}.parquet"
OUTPUT_XLSX = OUTPUT_DIR / f"events_sale_{DATE_FROM}_{DATE_TO}.xlsx"
...
def fetch_month(begin_str, end_str, retry=2):
...
def to_standard_schema(df):
    """转换为下游 inventory_events 标准 schema"""
    std = pd.DataFrame()
    ...
    # 销售单: ID号/名称 = 客户
    std['customer_id'] = df['ID号'].astype('string')
    std['customer_name'] = df['名称'].astype('string')
    std['supplier_id'] = pd.Series([None] * len(df), dtype='string')
    std['supplier_name'] = pd.Series([None] * len(df), dtype='string')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="boson ERP 销售明细抓取 (按月切 + 缓存 + parquet/xlsx)",
